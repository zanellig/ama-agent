...
export function FrequencyBarVisualizer({ analyzerNode, isActive }: FrequencyBarVisualizerProps) {
  ...
  useEffect(() => {
    ...
    const barCount = analyzerNode ? Math.floor(analyzerNode.frequencyBinCount / 10) : 16;
    ...
    const draw = () => {
      ...
      if (analyzerNode && isActive) {
        ...
        for (let i = 0; i < staticBarCount; i++) {
          ...
          ctx.beginPath();
          ctx.roundRect(barX, height - barHeight - 2, staticBarWidth, barHeight, 2);
          ctx.fill();
